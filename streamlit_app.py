...
# Process button
if st.button("Process Files", disabled=(results_file is None or fragments_file is None)):
    if results_file is None or fragments_file is None:
        ...
        try:
            ...
            results_suffix = ".parquet" if results_file.name.endswith(".parquet") else ".tsv"
            fragments_suffix = ".parquet" if fragments_file.name.endswith(".parquet") else ".tsv"
            
            with tempfile.NamedTemporaryFile(delete=False, suffix=results_suffix) as temp_results:
                temp_results.write(results_file.getvalue())
                results_path = temp_results.name
                
            with tempfile.NamedTemporaryFile(delete=False, suffix=fragments_suffix) as temp_fragments:
            ...
            download_data = create_download_link(output_df, output_filename, output_format)
            ...
            st.write(f"PSMs with ambiguity annotation: {output_df['ambiguity_sequence'].notna().sum()}")
